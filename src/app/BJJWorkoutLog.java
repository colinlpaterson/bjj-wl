...
public class BJJWorkoutLog {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            int choice = promptIntRange(in, "Choose an option (1-5): ", 1, 5);
        ...
        for (int i = 0; i < slice.size(); i++) {
            ...
            if (roundsBlock != null && !roundsBlock.trim().isEmpty()) {
                ...
                for (int rIndex = 0; rIndex < roundObjects.size(); rIndex++) {
                    ...
                    System.out.println("  Round " + (rIndex + 1) + ": belt="
                            + (belt == null ? "(unknown)" : belt)
                            + ", duration=" + dur + " min");
                }
            }
    ...
    private static void exportCsv(WorkoutRepository repo) throws Exception {
        ...
        // Write workouts.csv and rounds.csv (same logic as before)
        ...
        try (var wcsv = java.nio.file.Files.newBufferedWriter(workoutsCsv, java.nio.charset.StandardCharsets.UTF_8,
                                                              java.nio.file.StandardOpenOption.CREATE,
                                                              java.nio.file.StandardOpenOption.TRUNCATE_EXISTING);
             var rcsv = java.nio.file.Files.newBufferedWriter(roundsCsv, java.nio.charset.StandardCharsets.UTF_8,
                                                              java.nio.file.StandardOpenOption.CREATE,
                                                              java.nio.file.StandardOpenOption.TRUNCATE_EXISTING)) {

            wcsv.write("date,drills,roundsCount,notes\n");
            rcsv.write("date,roundIndex,beltLevel,partnerSize,partnerAge,timesYouWereSubmitted,submissionTypesAgainst,timesYouSubmittedPartner,submissionTypesFor,observations\n");
    ...
    private static Workout promptWorkout(Scanner input) {
        String date = promptDate(input);  // validated date
        ...
        for (int i = 1; i <= roundsCount; i++) {
            ...
            r.beltLevel = promptBeltLevel(input); // validated belt level
            ...
            r.roundDurationMinutes = promptIntRange(input,
                    "Round duration in minutes (1–15): ", 1, 15);

            r.timesYouWereSubmitted = promptIntRange(input,
                    "4) Times you were submitted (0–20): ", 0, 20);

            r.submissionTypesAgainst = promptString(input,
                    "5) Submission type(s) they used (comma-separated): ");

            r.timesYouSubmittedPartner = promptIntRange(input,
                    "6) Times you submitted partner (0–20): ", 0, 20);

            r.submissionTypesFor = promptString(input,
                    "7) Submission type(s) you used (comma-separated): ");

            r.observations = promptString(input,
                    "8) Observations of the round: ");
    ...
    private static String promptBeltLevel(Scanner input) {
        while (true) {
            ...
            switch (belt) {
                ...
                case "brown":
                case "black":
                    // return a normalized version (capitalized)
